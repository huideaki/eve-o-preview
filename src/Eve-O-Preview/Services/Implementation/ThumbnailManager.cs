...
namespace EveOPreview.Services
{
    sealed class ThumbnailManager : IThumbnailManager
    {
        ...
        public ThumbnailManager(IMediator mediator, IThumbnailConfiguration configuration, IProcessMonitor processMonitor, IWindowManager windowManager, IThumbnailViewFactory factory)
        {
            ...
            var keysRegistered = new List<string>();
            foreach (var item in this._configuration.CycleClientGroups)
            {
                if (keysRegistered.Any(x => item.CycleGroupBackwardHotkeys.Contains(x)) || keysRegistered.Any(x => item.CycleGroupForwardHotkeys.Contains(x)))
                {
                    throw new ArgumentException("Key is already registered");
                }

                RegisterCycleClientHotkey(item.CycleGroupForwardHotkeys.Select(x => this._configuration.StringToKey(x)), true, item.CycleGroupClientsOrder);
                RegisterCycleClientHotkey(item.CycleGroupBackwardHotkeys.Select(x => this._configuration.StringToKey(x)), false, item.CycleGroupClientsOrder);
                keysRegistered.AddRange(item.CycleGroupBackwardHotkeys);
                keysRegistered.AddRange(item.CycleGroupForwardHotkeys);
            }
        }
